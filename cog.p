...
class TournamentCog(commands.GroupCog, name="tournament"):
    ...
    def __init__(self, bot: "BallsDexBot"):
        ...
        # Set up Challonge API using environment variables
    ...
    async def _process_tournament(self, tournament: Tournament):
        ...
        try:
            ...
            if tournament_info["state"] == "complete":
                tournaments.finalize(tournament.challonge_id)
    ...
    async def _send_match_result(self, tournament: Tournament, winner: TournamentPlayer, loser: TournamentPlayer, battle_result: dict):
        ...
        try:
            ...
            embed = discord.Embed(
                title="⚔️ Tournament Match Result",
    ...
    async def _handle_tournament_complete(self, tournament: Tournament):
        ...
        try:
            ...
            if champion:
                ...
                # Show champion's team
                if champion.balls:
                    champion_team = "\n".join([f"• {ball.emoji} {ball.country}" for ball in champion.balls])
                    embed.add_field(name="Champion's Team", value=champion_team, inline=False)
    ...
    async def create(
        ...
        try:
            challonge_tournament = challonge.tournaments.create(
                ...
                description=f"BallsDex Tournament: {name}",
                private=True
            )
        except Exception as e:
        ...
        await interaction.followup.send(
            f"🏆 Tournament **{name}** created! Registration is now open.",
            embed=embed,
            view=view
        )
